...
class IncomingForm:
    ...
    def setup_window(self):
        """إعداد نافذة النموذج"""
        self.parent.title("نموذج تسجيل وارد" if not self.record_id else "تعديل سجل وارد")
    ...
    def create_record_fields(self, parent):
        ...
        ttk.Label(row1, text="رقم الوارد:").pack(side=tk.RIGHT, padx=5)
        self.incoming_number = ttk.Entry(row1, width=20)
        self.incoming_number.pack(side=tk.RIGHT, padx=5)
        ...
        ttk.Label(row3, text="جهة الوارد:").pack(side=tk.RIGHT, padx=5)
        self.source_combo = ttk.Combobox(row3, width=25, state="readonly")
        self.source_combo.pack(side=tk.RIGHT, padx=5)
        
        ttk.Label(row3, text="نوع الوارد:").pack(side=tk.RIGHT, padx=5)
        self.type_combo = ttk.Combobox(row3, width=25, state="readonly")
        self.type_combo.pack(side=tk.RIGHT, padx=5)
        
        # الصف الرابع
        row4 = ttk.Frame(parent)
        row4.pack(fill=tk.X, pady=2)
        
        ttk.Label(row4, text="الموظف:").pack(side=tk.RIGHT, padx=5)
        self.employee_combo = ttk.Combobox(row4, width=25, state="readonly")
    ...
    def load_reference_data(self):
        ...
        # جهات الوارد
        sources = self.db_manager.execute_query("SELECT id, name FROM incoming_sources ORDER BY name")
        self.source_combo['values'] = [name for _, name in sources]
        self.sources_dict = {name: id for id, name in sources}
        
        # أنواع الوارد
        types = self.db_manager.execute_query("SELECT id, name FROM incoming_types ORDER BY name")
        self.type_combo['values'] = [name for _, name in types]
        self.types_dict = {name: id for id, name in types}
    ...
    def generate_record_numbers(self):
        ...
        last_record = self.db_manager.execute_query(
            "SELECT record_number FROM incoming_records WHERE record_number LIKE ? ORDER BY id DESC LIMIT 1",
            (f'IN-{year}-%',)
        )
        ...
        self.record_number.insert(0, f'IN-{year}-{new_number:04d}')
        
        # توليد رقم وارد (يمكن تعديله يدوياً)
        self.incoming_number.delete(0, tk.END)
        self.incoming_number.insert(0, f'IN-{datetime.now().strftime("%Y%m%d")}')
        
        # توليد رقم تسلسلي
        last_serial = self.db_manager.execute_query(
            "SELECT serial_number FROM incoming_records ORDER BY id DESC LIMIT 1"
        )
        if last_serial:
    ...
    def load_record_data(self):
        ...
        query = """
        SELECT record_number, incoming_number, serial_number, title,
               incoming_source_id, incoming_type_id, employee_id, specialization_id,
               registration_date, details
        FROM incoming_records WHERE id = ?
        """
        ...
        if record:
            ...
            self.incoming_number.insert(0, data[1])
            self.serial_number.insert(0, data[2])
            self.title_entry.insert(0, data[3])
            self.registration_date.delete(0, tk.END)
            self.registration_date.insert(0, data[8])
            self.details_text.insert('1.0', data[9] if data[9] else '')
            
            # تحميل القوائم المنسدلة
            self.load_combo_values(data[4], data[5], data[6], data[7])
            
            # تحميل المرفقات
            self.load_attachments()
    
    def load_combo_values(self, source_id, type_id, employee_id, specialization_id):
        """تحميل قيم القوائم المنسدلة بناءً على الـ IDs"""
        # جهة الوارد
        source_name = self.db_manager.execute_query(
            "SELECT name FROM incoming_sources WHERE id = ?", (source_id,)
        )
        if source_name:
            self.source_combo.set(source_name[0][0])
        
        # نوع الوارد
        type_name = self.db_manager.execute_query(
            "SELECT name FROM incoming_types WHERE id = ?", (type_id,)
        )
        if type_name:
            self.type_combo.set(type_name[0][0])
    ...
    def load_attachments(self):
        ...
        attachments = self.db_manager.execute_query(
            "SELECT id, file_name, file_path, description FROM attachments WHERE record_id = ? AND record_type = 'incoming'",
            (self.record_id,)
        )
    ...
    def add_attachment(self):
        ...
        if file_path:
            ...
            if self.record_id:
                ...
                saved_file = self.file_manager.save_attachment(file_path, self.record_id, 'incoming')
                if saved_file:
    ...
    def validate_form(self):
        ...
        if not self.incoming_number.get().strip():
            messagebox.showerror("خطأ", "رقم الوارد مطلوب")
            return False
        ...
        if not self.source_combo.get():
            messagebox.showerror("خطأ", "جهة الوارد مطلوبة")
            return False
        
        if not self.type_combo.get():
            messagebox.showerror("خطأ", "نوع الوارد مطلوب")
            return False
    ...
    def save_record(self):
        ...
        try:
            ...
            data = (
                ...
                self.incoming_number.get().strip(),
                self.serial_number.get().strip(),
                self.title_entry.get().strip(),
                self.sources_dict[self.source_combo.get()],
                self.types_dict[self.type_combo.get()],
            ...
            if self.record_id:
                ...
                SET record_number=?, incoming_number=?, serial_number=?, title=?,
                    incoming_source_id=?, incoming_type_id=?, employee_id=?, specialization_id=?,
                    registration_date=?, details=?
                WHERE id=?
                """
                ...
                (record_number, incoming_number, serial_number, title, incoming_source_id,
                 incoming_type_id, employee_id, specialization_id, registration_date, details)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """
                ...
                for att in self.attachments:
                    if 'file_path' in att and not att.get('id'):
                        saved_file = self.file_manager.save_attachment(
                            att['file_path'], self.record_id, 'incoming'
                        )
                        if saved_file:
                            query = """
                            INSERT INTO attachments (record_id, record_type, file_name, file_path, description)
